...
import { clamp, createRange, getDecimals, propsFactory } from '@/utils'

// Types
import type { ExtractPropTypes, InjectionKey, PropType, Ref } from 'vue'
import type { SPSliderTrack } from './SPSliderTrack'
...
type SliderProvide = {
  ...
  tickSize: Ref<number>
  trackContainerRef: Ref<SPSliderTrack | undefined>
...
export const SPSliderSymbol: InjectionKey<SliderProvide> =
  Symbol.for('speed:sp-slider')
...
export const makeSliderProps = propsFactory(
  {
    ...
    ticks: {
      type: [Array, Object] as PropType<
        readonly number[] | Record<number, string>
      >,
    },
    tickSize: {
      type: [Number, String],
...
export const useSteps = (props: SliderProps) => {
  const min = computed(() => parseFloat(String(props.min)))
  const max = computed(() => parseFloat(String(props.max)))
  const step = computed(() =>
    Number(props.step) > 0 ? parseFloat(String(props.step)) : 0
  )
  const decimals = computed(() =>
    Math.max(getDecimals(step.value), getDecimals(min.value))
  )
  ...
  function roundValue(value: string | number) {
    ...
    let newValue =
      Math.round((clamped - offset) / step.value) * step.value + offset
...
export const useSlider = ({
  ...
  const thumbSize = computed(() => parseInt(String(props.thumbSize), 10))
  const tickSize = computed(() => parseInt(String(props.tickSize), 10))
  const trackSize = computed(() => parseInt(String(props.trackSize), 10))
  const numTicks = computed(() => (max.value - min.value) / step.value)
  const disabled = toRef(() => props.disabled)

  const thumbColor = computed(() =>
    props.error || props.disabled ? undefined : props.thumbColor ?? props.color
  )
  const trackColor = computed(() =>
    props.error || props.disabled ? undefined : props.trackColor ?? props.color
  )
  const trackFillColor = computed(() =>
    props.error || props.disabled
      ? undefined
      : props.trackFillColor ?? props.color
  )

  const mousePressed = shallowRef(false)

  const startOffset = shallowRef(0)
  const trackContainerRef = ref<SPSliderTrack | undefined>()
  ...
  function parseMouseMove(e: MouseEvent | TouchEvent): number | void {
    ...
    // 仅在拖拽开始时获取 rect，拖拽过程中复用
    if (!cachedTrackRect || !mousePressed.value) {
      cachedTrackRect = el.getBoundingClientRect()
    }
    ...
    const { [start]: trackStart, [length]: trackLength } = cachedTrackRect
    const clickOffset = getPosition(e, position)

    // It is possible for left to be NaN, force to number
    let clickPos =
      clamp((clickOffset - trackStart - startOffset.value) / trackLength) || 0

    if (vertical ? indexFromEnd.value : indexFromEnd.value !== isRtl.value)
      clickPos = 1 - clickPos
  ...
  function onSliderTouchstart(e: TouchEvent) {
    ...
    e.target?.addEventListener('touchend', onSliderTouchend as EventListener, {
      passive: false,
    })
  }
  ...
  const parsedTicks = computed<Tick[]>(() => {
    ...
    if (!props.ticks) {
      return numTicks.value !== Infinity
        ? createRange(numTicks.value + 1).map(t => {
  ...
  const hasLabels = computed(() =>
    parsedTicks.value.some(({ label }) => !!label)
  )
