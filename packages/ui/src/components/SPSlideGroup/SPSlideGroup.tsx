...
export const SPSlideGroupSymbol: InjectionKey<GroupProvide> = Symbol.for(
  'speed:sp-slide-group'
)
...
type SPSlideGroupSlots = {
  default: SlideGroupSlot
  prev: SlideGroupSlot
  next: SlideGroupSlot
}

export const makeSPSlideGroupProps = propsFactory(
  {
    ...
    ...makeTagProps(),
    ...makeGroupProps({
      selectedClass: 'sp-slide-group-item--active',
    }),
  },
  'SPSlideGroup'
)

export const SPSlideGroup = genericComponent<
  new <T>(
    props: {
      modelValue?: T
      'onUpdate:modelValue'?: (value: T) => void
    },
    slots: SPSlideGroupSlots
  ) => GenericProps<typeof props, typeof slots>
>()({
  name: 'SPSlideGroup',

  props: makeSPSlideGroupProps(),
      ...
      return (
        <Tag
          class={[
            'sp-slide-group',
            {
              'sp-slide-group--vertical': !isHorizontal.value,
              'sp-slide-group--has-affixes': hasAffixes.value,
              'sp-slide-group--is-overflowing': isOverflowing.value,
            },
        ...
        >
          {hasAffixes.value && (
            <div
              key="prev"
              class={[
                'sp-slide-group__prev',
                { 'sp-slide-group__prev--disabled': !hasPrev.value },
              ]}
            ...
            >
              {slots.prev?.(slotProps.value) ?? (
                ...
                <SpIcon
                  name={
                    typeof (isRtl.value ? props.nextIcon : props.prevIcon) ===
                    'string'
                      ? ((isRtl.value
                          ? props.nextIcon
                          : props.prevIcon) as string)
                      : String(
                          (isRtl.value ? props.nextIcon : props.prevIcon) || ''
                        )
                  }
                ></SpIcon>
                // </VFadeTransition>
          ...
          <div
            key="container"
            ref={containerRef}
            class={['sp-slide-group__container', props.contentClass]}
            onScroll={onScroll}
          >
            <div
              ref={contentRef}
              class="sp-slide-group__content"
              onFocusin={onFocusin}
          ...
          {hasAffixes.value && (
            <div
              key="next"
              class={[
                'sp-slide-group__next',
                { 'sp-slide-group__next--disabled': !hasNext.value },
              ]}
              onMousedown={onFocusAffixes}
            ...
            >
              {slots.next?.(slotProps.value) ?? (
                ...
                <SpIcon
                  name={
                    typeof (isRtl.value ? props.prevIcon : props.nextIcon) ===
                    'string'
                      ? ((isRtl.value
                          ? props.prevIcon
                          : props.nextIcon) as string)
                      : String(
                          (isRtl.value ? props.prevIcon : props.nextIcon) || ''
                        )
                  }
                ></SpIcon>
                // </VFadeTransition>
...
export type SPSlideGroup = InstanceType<typeof SPSlideGroup>
