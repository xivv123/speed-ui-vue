...
import { Box, getElementBox, getOverflow, getTargetBox } from '@/utils/box'
...
export const makeLocationStrategyProps = propsFactory(
  {
    locationStrategy: {
      ...
      default: 'static',
      validator: (val: any) =>
        typeof val === 'function' || val in locationStrategies,
    },
    location: {
    ...
    useToggleScope(
      ...
      reset => {
        ...
        if (typeof props.locationStrategy === 'function') {
          updateLocation.value = props.locationStrategy(
            data,
            props,
            contentStyles
          )?.updateLocation
        } else {
          updateLocation.value = locationStrategies[props.locationStrategy](
            data,
            props,
            contentStyles
          )?.updateLocation
...
function connectedLocationStrategy(
  ...
  function updateLocation() {
    ...
    if (!scrollParents.length) {
      ...
      if (
        !(data.contentEl.value.style.top && data.contentEl.value.style.left)
      ) {
        contentBox.x -= parseFloat(
          document.documentElement.style.getPropertyValue(
            '--sp-body-scroll-x'
          ) || '0'
        )
        contentBox.y -= parseFloat(
          document.documentElement.style.getPropertyValue(
            '--sp-body-scroll-y'
          ) || '0'
        )
    ...
    Object.assign(contentStyles.value, {
      '--sp-overlay-anchor-origin': `${placement.anchor.side} ${placement.anchor.align}`,
      ...
      minWidth: convertToUnit(
        axis === 'y'
          ? Math.min(minWidth.value, targetBox.width)
          : minWidth.value
      ),
