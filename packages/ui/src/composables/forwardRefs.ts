...
type OmitPrefix<T, P extends string, E = Extract<keyof T, `${P}${any}`>> = [
  E
] extends [never]
  ? T
  : Omit<T, `${P}${any}`>
...
export function forwardRefs<
  T extends {},
  U extends NonEmptyArray<
    Ref<
      | HTMLElement
      | Omit<ComponentPublicInstance, '$emit' | '$slots'>
      | undefined
    >
  >,
    ...
    getOwnPropertyDescriptor(target, key) {
      ...
      for (const ref of refs) {
        ...
        const descriptor =
          ...
          ('_' in ref.value
            ? getDescriptor((ref.value._ as any)?.setupState, key)
            : undefined)
        if (descriptor) return descriptor
      }
