...
import type { SelectItemKey } from '@/utils'
...
export function useVirtual<T>(props: VirtualProps, items: Ref<readonly T[]>) {
  ...
  watchEffect(() => {
    itemHeight.value = parseFloat(String(props.itemHeight || 0))
  })
  ...
  const last = shallowRef(
    Math.ceil(
      ...
      (parseInt(String(props.height!)) || display.height.value) /
        (itemHeight.value || 16)
    ) || 1
  )
  ...
  const viewportHeight = computed(() => {
    return containerRef.value === document.documentElement
      ...
      : contentRect.value?.height || parseInt(String(props.height!)) || 0
  })
  ...
  function _calculateVisibleItems() {
    if (!containerRef.value || !viewportHeight.value || !itemHeight.value)
      return
