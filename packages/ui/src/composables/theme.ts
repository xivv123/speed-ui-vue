...
function genDefaults() {
  return {
    ...
    stylesheetId: 'speed-theme-stylesheet',
    scoped: false,
    unimportant: false,
    utilities: true,
  }
...
function genCssVariables(theme: InternalThemeDefinition, prefix: string) {
  ...
  for (const [key, value] of Object.entries(theme.colors)) {
    ...
    if (!key.startsWith('on-')) {
      variables.push(
        `--${prefix}theme-${key}-overlay-multiplier: ${
...
function genVariation(
  name: string,
  color: string,
  variations: VariationsOptions | false
) {
  ...
  if (variations) {
    for (const variation of ['lighten', 'darken'] as const) {
      ...
      for (const amount of createRange(variations[variation], 1)) {
        object[`${name}-${variation}-${amount}`] = RGBtoHex(
...
function genVariations(
  colors: InternalThemeDefinition['colors'],
  variations: VariationsOptions | false
) {
...
export function createTheme(
  ...
  const styles = computed(() => {
    ...
    createCssClass(
      lines,
      ':root',
      genCssVariables(current.value, parsedOptions.prefix),
      parsedOptions.scope
    )
    ...
    for (const [themeName, theme] of Object.entries(computedThemes.value)) {
      createCssClass(
        lines,
        `.${parsedOptions.prefix}theme--${themeName}`,
        [
      ...
      for (const key of colors) {
        if (key.startsWith('on-')) {
          createCssClass(
            ...
            `.${key}`,
            [
              `color: rgb(var(--${parsedOptions.prefix}theme-${key}))${important}`,
            ],
            parsedOptions.scope
          )
        } else {
          ...
          createCssClass(
            ...
            `.${scoped}border-${key}`,
            [
              `--${parsedOptions.prefix}border-color: var(--${parsedOptions.prefix}theme-${key})`,
            ],
            parsedOptions.scope
          )
        }
  ...
  const themeClasses = toRef(() =>
    parsedOptions.isDisabled
      ? undefined
      : `${parsedOptions.prefix}theme--${name.value}`
  )
  ...
  function install(app: App) {
    ...
    const head = app._context.provides.usehead as any // | (HeadClient & VueHeadClient<any>)
    if (head) {
      function getHead() {
        return {
          style: [
  ...
  function change(themeName: string) {
    if (!themeNames.value.includes(themeName)) {
      consoleWarn(`Theme "${themeName}" not found on the Speed theme instance`)
      return
    }
  ...
  function cycle(themeArray: string[] = themeNames.value) {
    ...
    const nextIndex =
      currentIndex === -1 ? 0 : (currentIndex + 1) % themeArray.length
...
export function provideTheme(props: { theme?: string }) {
  ...
  if (!theme) throw new Error('Could not find Speed theme injection')

  const name = toRef(() => props.theme ?? theme.name.value)
  const current = toRef(() => theme.themes.value[name.value])

  const themeClasses = toRef(() =>
    theme.isDisabled ? undefined : `${theme.prefix}theme--${name.value}`
  )
