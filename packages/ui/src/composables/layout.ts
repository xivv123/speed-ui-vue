...
export const SpeedLKey: InjectionKey<LayoutProvide> =
  Symbol.for('speed:layout')
export const SpeedLayoutItemKey: InjectionKey<{ id: string }> = Symbol.for(
  'speed:layout-item'
)
...
export function useLayout() {
  const layout = inject(SpeedLKey)

  if (!layout) throw new Error('[Speed] Could not find injected layout')
...
const generateLayers = (
  ...
  for (const id of layout) {
    ...
    const layer = {
      ...previousLayer,
      [position.value]:
        parseInt(String(previousLayer[position.value]), 10) +
...
export function createLayout(props: {
  ...
  const parentLayout = inject(SpeedLKey, null)
  ...
  const computedOverlaps = computed(() => {
    ...
    for (const overlap of overlaps.filter(item => item.includes(':'))) {
      ...
      map.set(bottom, {
        ...
        amount: parseInt(String(topAmount.value), 10),
      })
      map.set(top, {
        position: bottomPosition.value,
        amount: -parseInt(String(bottomAmount.value), 10),
      })
    }
      ...
      const layoutItemStyles = computed<CSSProperties>(() => {
        ...
        const styles = {
          ...
          transform: `translate${isHorizontal ? 'X' : 'Y'}(${
            (active.value ? 0 : -(size === 0 ? 100 : size)) *
        ...
        if (!item) consoleWarn(`[Speed] Could not find layout item "${id}"`)
