...
import { consoleError, getCurrentInstance, propsFactory } from '@/utils'
...
export const VNestedSymbol: InjectionKey<NestedProvide> =
  Symbol.for('speed:nested')
...
export const useNested = (props: NestedProps) => {
  ...
  const selected = useProxiedModel(
    ...
    props.selected,
    v =>
      selectStrategy.value.in(v, children.value, parents.value, disabled.value),
...
export const useNestedItem = (
  ...
  const item = {
    ...
    activate: (activated: boolean, e?: Event) =>
      parent.root.activate(computedId.value, activated, e),
    isActivated: computed(() =>
      parent.root.activated.value.has(computedId.value)
    ),
    select: (selected: boolean, e?: Event) =>
      parent.root.select(computedId.value, selected, e),
    isSelected: computed(
      () => parent.root.selected.value.get(computedId.value) === 'on'
    ),
    isIndeterminate: computed(
      () => parent.root.selected.value.get(computedId.value) === 'indeterminate'
    ),
