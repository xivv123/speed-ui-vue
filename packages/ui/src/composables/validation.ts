...
import type { EventProp, MaybeRef } from '../utils'
...
// 临时定义 ValidationAlias 类型，直到 labs/rules 可用
export type ValidationAlias = string
...
export function useValidation(
  ...
  // 临时使用 props.rules 直到 useRules 可用
  ...
  const isDirty = computed(
    () =>
      !!(
        ...
        wrapInArray(validationModel.value === '' ? null : validationModel.value)
          .length
      )
  )
  const errorMessages = computed(() => {
    return props.errorMessages?.length
      ? wrapInArray(props.errorMessages)
          ...
          .slice(0, Math.max(0, Number(props.maxErrors)))
  ...
  const isValid = computed(() => {
    ...
    if (isPristine.value) {
      return internalErrorMessages.value.length || validateOn.value.lazy
        ? null
        : true
    } else {
  ...
  useToggleScope(
    () =>
      ...
      (validateOn.value.invalidInput && isValid.value === false),
    () => {
