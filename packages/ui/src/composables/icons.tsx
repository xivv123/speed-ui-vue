...
export interface IconAliases {
  ...
  edit: IconValue
  heart: IconValue
  heartOutline: IconValue
  star: IconValue
  starOutline: IconValue
  thumbUp: IconValue
  thumbUpOutline: IconValue
...
export const IconSymbol: InjectionKey<InternalIconOptions> =
  Symbol.for('speed:icons')
...
export function createIcons(options?: IconOptions): InternalIconOptions {
  ...
  const defaultSet = options?.defaultSet ?? 'svg'

  return mergeDeep(
    {
      defaultSet,
      sets,
      aliases: svgIconAliases,
    },
    options
  ) as InternalIconOptions
}

export function useIcon(props: MaybeRefOrGetter<IconValue | undefined>) {
  const icons = inject(IconSymbol)

  if (!icons) throw new Error('Could not find Speed icon injection')
